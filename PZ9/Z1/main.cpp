...
unsigned int hashfja(int ulaz, unsigned int max) {
	return ulaz%max;
}
...
private:
    int max;
    int velicina;
    unsigned int(*pocNaFun)(Tip1, unsigned int);
...
template<typename Tip1, typename Tip2>
HashMapa<Tip1, Tip2>::~HashMapa()
{
    delete[] niz;
}
template<typename Tip1, typename Tip2>
HashMapa<Tip1, Tip2>::HashMapa(const HashMapa& m)
{
    max=m.max;
    velicina=m.velicina;
    ...
    for(int i=0; i<max; i++)
    {
        niz[i]=std::make_pair(m.niz[i].first, m.niz[i].second);
    }
}

template<typename Tip1, typename Tip2>
void HashMapa<Tip1, Tip2>::obrisi(const Tip1& kljuc)
{
    for(int i=0; i<max; i++)
    {
...
template<typename Tip1, typename Tip2>
HashMapa<Tip1, Tip2>& HashMapa<Tip1, Tip2>::operator=(const HashMapa& m)
{
...
template<typename Tip1, typename Tip2>
Tip2& HashMapa<Tip1, Tip2>::operator[](Tip1 kljuc)
{
    if (pocNaFun==nullptr) throw std::logic_error("Nije unesena hash funkcija");
    for(int i=0; i<max; i++)
    {
        if(niz[i].first==kljuc) return niz[i].second;
    }
    int indeks=pocNaFun(kljuc, max);
    for(;;)
        {
            if(indeks<max && niz[indeks].first!=Tip1())
            {
                indeks++;
            }
            else break;
    ...
    if(indeks>=max)
    {
        std::pair<Tip1, Tip2>* pomocni=new std::pair<Tip1, Tip2>[max+100];
        for(int i=0; i<max; i++)
        {
            pomocni[i]=std::make_pair(niz[i].first, niz[i].second);
        }
        delete[] niz;
        niz=pomocni;
        max+=100;
    ...
    velicina++;
    niz[indeks]=std::make_pair(kljuc, Tip2());
    return niz[indeks].second;
}


template<typename Tip1, typename Tip2>
Tip2 HashMapa<Tip1, Tip2>::operator[](Tip1 kljuc) const
{
    for(int i=0; i<max; i++)
    {
        if(niz[i].first==kljuc) return niz[i].second;
...
HashMapa<Tip1, Tip2>::HashMapa()
{
    niz=new std::pair<Tip1, Tip2>[200];
    max=200;
    velicina=0;
    pocNaFun=nullptr;
}
...
template<typename Tip1, typename Tip2>
void HashMapa<Tip1, Tip2>::definisiHashFunkciju(unsigned int(*fun)(Tip1, unsigned int))
{
    pocNaFun=fun;
}

template<typename Tip1, typename Tip2>
int HashMapa<Tip1, Tip2>::brojElemenata()
{
    return velicina;
}

template<typename Tip1, typename Tip2>
int HashMapa<Tip1, Tip2>::brojElemenata() const
{
    return velicina;
}
...
void testni_program()
{
    ...
    HashMapa<int, int>hash_mapa;
    hash_mapa.definisiHashFunkciju(hashfja);
    srand(time(NULL));
    clock_t t1=clock();
    ...
    t2=clock();
    std::cout<<"Vrijeme upisivanja u HashMapu: "<<(t2-t1)/(CLOCKS_PER_SEC/1000.)<<std::endl;
    ...
    */
    ...
    t2=clock();
    std::cout<<"Vrijeme pristupa elementima HashMape: "<<(t2-t1)/(CLOCKS_PER_SEC/1000.)<<std::endl;
    ...
    */
    ...
    t2=clock();
    std::cout<<"Vrijeme brisanja elemenata NizMape: "<<(t2-t1)/(CLOCKS_PER_SEC/1000.)<<std::endl;
    srand(time(NULL));
    t1=clock();
    for(int i=0; i<4000; i++)
    {
        binarno_stablo_mapa.obrisi(i);
    }
    t2=clock();
    std::cout<<"Vrijeme brisanja elemenata BinStabloMape: "<<(t2-t1)/(CLOCKS_PER_SEC/1000.)<<std::endl;
    srand(time(NULL));
    t1=clock();
    for(int i=0; i<4000; i++)
    {
    ...
    t2=clock();
    std::cout<<"Vrijeme brisanja elemenata HashMape: "<<(t2-t1)/(CLOCKS_PER_SEC/1000.)<<std::endl;
}
